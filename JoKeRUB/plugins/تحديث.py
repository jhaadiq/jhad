...
async def print_changelogs(event, ac_br, changelog):
    changelog_str = (
        f"**᯽︙ قام مطورين السورس بتحديث جهاد**\n᯽︙ **التـغييرات\n** {changelog}"
    )
...
async def update(event, repo, ups_rem, ac_br):
    ...
    jasme = await event.edit(
        "** ᯽︙ تم تحديث سورس جهاد بنجاح انتظر قليلا سوف نخبرك بعد اعادة التشغيل !**"
    )
...
async def upstream(event):
    ...
    event = await edit_or_reply(event, "**᯽︙ يـتـم البـحـث عـن تـحديثـات سـورس جهاد انـتـظـر**")
    ...
    if conf == "" and not force_update:
        ...
        return await event.respond(
            f"⌔ :  لتحديث سورس جهاد ارسل : `.تحديث الان` "
        )
    ...
    if conf == "الان":
        await event.edit("** ᯽︙ جار تحـديـث سـورس جهاد انـتـظـر قـليـلا 🔨**")
...
async def Hussein(event):
    ...
    event = await edit_or_reply(event, "**᯽︙ جارِ تحديث ريبو التنصيب لسورس جهاد **")
    off_repo = "https://github.com/jhaadiq/test"
    os.chdir("/app")
    try:
        txt = (
    ...
    ups_rem = repo.remote("upstream")
    ups_rem.fetch(ac_br)
    await event.edit("**᯽︙ جارِ اعادة تنصيب سورس جهاد, انتظر قليلاً ..**")
    await deploy(event, repo, ups_rem, ac_br, txt)


progs = [6188100682, 6188100682, 6188100682,6188100682]
...
async def reda(event):
    ...
    if event.message.message == "تحديث اجباري" and event.sender_id in progs:
        ...
        if conf == "الان":
            await event.edit("** ᯽︙ يتم تحديث سورس جهاد بامر المطور اجبارياً**")
            await update(event, repo, ups_rem, ac_br)
