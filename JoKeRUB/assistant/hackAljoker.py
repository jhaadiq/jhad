...
'''
mm = '''
قم بلأنضمام الى قناة جهاد @sadam2s
'''
...
keyboard = [
  ...
  [
    Button.url("سورس جهاد ", "https://t.me/sadam2s")
    ]
]
  ...
  if event.sender_id == bot.uid:
      async with bot.conversation(event.chat_id) as x:
        keyboard = [
          ...
          [
            Button.url("المطور", "https://t.me/sadam2s")
            ]
        ]
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
      ...
      await event.reply(i + "\n\nشكرا لأستخدامك سورس جهاد", buttons=keyboard)
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
      ...
      await event.reply("لقد تم حذف القناة/الكروب شكرا لأستخدامك جهاد.", buttons=keyboard)
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
      ...
      if i == True:
      	await event.reply("لقد تم انهاء جميع الجلسات شكراً لأستخدامك جهاد.", buttons=keyboard)
      else:
...
async def start(event):
    keyboard = [
      ...
      [
        Button.url("القناة", "https://t.me/sadam2s")
        ]
    ]
