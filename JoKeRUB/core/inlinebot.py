...
def main_menu():
    text = f"**- [سـورس جهاد](https://t.me/sadam2s)\
        \n المسـاعد\
        \n\
        \n◽ جهاد لـ {mention}**"
    buttons = [
...
async def inline_handler(event):  # sourcery no-metrics
    ...
    if query_user_id == Config.OWNER_ID or query_user_id in Config.SUDO_USERS:
        ...
        if query.startswith("l313l"):
            buttons = [
                (
                    ...
                    Button.url(" JoKeRUB UsᴇʀBoᴛ", "https://t.me/sadam2s"),
                )
            ]
            ...
            result = builder.article(
                title="رسـالة تـرول من سـورس جهاد 🧸♥",
            ...
            result = builder.article(
                title="همسـة سـرية من سـورس جهاد 🧸♥",
                text=f"فقـط {razan} هـو مـن يستـطيع مشـاهدتهـا !",
                buttons=buttons,
            )
            await event.answer([result] if result else None)
            ...
            result = builder.article(
                title="رسـالة مخفيـة من سـورس جهاد 🧸♥",
                text=f"✖✖✖",
                buttons=buttons,
            )
            await event.answer([result] if result else None)
        ...
        buttons = [
            (
                Button.url("قنـاة السـورس ⚒️", "https://t.me/sadam2s"),
                Button.url(
                    "كـروب المـساعدة 📬",
                    "https://t.me/sadam2s1",
                ),
        ...
        text, msg_entities = await event.client._parse_message_text(
            "**[بـوت جهاد 🧸♥](https://t.me/sadam2s)**\
            \n\
            \n❤ جهاد هـو بـوت بسـيط يدخـل الـى حسـابك لجعـلك تتحـكم به معـ اوامـر معينـة.\
            \n\
            \n**اذا كـنت تـرغب بتـنصيـب السـورس عـلى حسـابك ,\
            \n🐾 تعال الـى [كروب المسـاعدة](https://t.me/sadam2s1)!**",
            "md",
        )
        result = types.InputBotInlineResult(
            id=str(uuid4()),
